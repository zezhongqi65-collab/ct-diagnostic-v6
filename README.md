...
- **个性化干预出题**：按学生薄弱维度自动组「一人一单」＋分发清单，**跨周不重复**（自动跳过已发过的题）
...
├── data/
│   └── question_bank.json              # 思维训练母题题库（150 道，五维各 30 道）
...
3. 下载 Word 题单（按薄弱维度分组），打印发给对应学生。
   - 题目为不插电思维训练题，纸笔可做、无需电脑，难度按结构属性分 1–3 星。
   - 题库预置 150 道母题（每维度 30 道）；系统记录已发过的题，**跨周自动跳过不重复**，题库不足时明确提示。
   - 「一人一单」：按批量诊断结果，为每名学生按薄弱维度（每周最多 2 维）自动组一份个人题单，并生成分发清单。
